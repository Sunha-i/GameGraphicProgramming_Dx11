...
HRESULT InitDevice()
{
	...
	hr = CompileShaderFromFile(L"../Library/Lab.fx", "VS", "vs_5_0", &pVertexShaderBlob);
	if (FAILED(hr))
		return hr;
	...
	// 5. compile pixel shader
	hr = CompileShaderFromFile(L"../Library/Lab.fx", "PS", "ps_5_0", &pPixelShaderBlob);
	if (FAILED(hr))
		return hr;
